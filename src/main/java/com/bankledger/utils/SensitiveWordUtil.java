...
public class SensitiveWordUtil {
    ...
    private static void initSensitiveWords(){
		  try {
			  InputStream in = SensitiveWordUtil.class.getClassLoader().getResourceAsStream("sensitiveword.txt");
		  	  byte[] all = new byte[in.available()];
		  	  in.read(all, 0, all.length);
		  	  String[] sensitiveWords = new String( all ).split("\n");
     ...
     * @return hexStr
     */
    public static String filteStr( String hexStr ){
    	String text = new String(  HexUtils.decodeHex(hexStr.toCharArray()) );
    	String filterStr = SensitiveWordUtil.replaceSensitiveWord(text, '*');
    	System.out.println("过滤后的字符串--->"+filterStr);
    	return HexUtils.encodeHexStr(filterStr.getBytes());
    }
    
    public static void main(String[] args) throws Exception {


            System.out.println(SensitiveWordUtil.filteStr(HexUtils.encodeHexStr("铁头要制服诱惑".getBytes())));
